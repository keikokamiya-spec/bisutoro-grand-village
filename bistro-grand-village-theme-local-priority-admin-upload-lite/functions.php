...
function bgv_webp_asset_path($path = '') {
  if ($path === '') {
    return '';
  }
  ...
  return $filename === '' ? '' : $dirname . $filename . '.webp';
}

function bgv_preferred_asset_path($path = '') {
  if ($path === '') {
    return '';
  }

  $webp_path = bgv_webp_asset_path($path);
  if ($webp_path && bgv_asset_exists($webp_path)) {
    return $webp_path;
  }

  return bgv_asset_exists($path) ? $path : '';
}

function bgv_preferred_asset_uri_if_exists($path = '') {
  $preferred_path = bgv_preferred_asset_path($path);
  return $preferred_path ? bgv_asset_uri($preferred_path) : '';
}

function bgv_preferred_asset_uri($path = '') {
  $preferred_path = bgv_preferred_asset_path($path);
  if ($preferred_path) {
    return bgv_asset_uri($preferred_path);
  }

  return $path !== '' ? bgv_asset_uri($path) : '';
}
...
function bgv_enqueue_assets() {
  ...
  wp_enqueue_style('bgv-base', bgv_asset_uri('assets/css/base.css'), array('bgv-style'), BGV_THEME_VERSION);

  wp_deregister_script('jquery');
  wp_register_script('jquery', bgv_asset_uri('assets/js/jquery.min.js'), array(), BGV_THEME_VERSION, true);
  ...
  wp_enqueue_script('bgv-custom', bgv_asset_uri('assets/js/custom.js'), array('jquery', 'bgv-bootstrap', 'bgv-flexslider', 'bgv-magnific-popup', 'bgv-masonry'), BGV_THEME_VERSION, true);
}
...
function bgv_linked_name($name, $image = '', $url = '') {
  ...
  $href = $url ? $url : $image;
  if ($href) {
    $class = ($url === '' && $image !== '') ? ' class="gallery"' : '';
    return '<a href="' . esc_url($href) . '"' . $class . '>' . $name . '</a>';
  }
  return $name;
}
...
function bgv_default_gallery_files() {
  return array(
    ...
    'IMG_0069.JPG', 'IMG_0164.jpg', 'IMG_0401.JPG', 'IMG_0660.jpg', 'IMG_0926.jpg', 'IMG_1013.jpg',
    'IMG_1014.jpg', 'IMG_1038.jpg', 'IMG_1055.jpg', 'IMG_1059.jpg', 'IMG_1597.jpg', 'IMG_1680.jpg',
    'IMG_2240.jpg', 'IMG_2252.JPG', 'IMG_2253.jpg', 'IMG_2254.JPG', 'IMG_2256.jpg', 'IMG_2259.JPG',
    'IMG_2260.JPG', 'IMG_2269.jpg', 'IMG_2339.JPG', 'IMG_2340.JPG', 'IMG_2371.JPG', 'IMG_2393.JPG',
    'IMG_2394.JPG', 'IMG_2396.JPG', 'IMG_2437.JPG', 'IMG_2445.JPG', 'IMG_7034.jpg', 'IMG_7265.JPG',
    'IMG_7312.JPG', 'IMG_7356.jpg', 'IMG_7368.jpg', 'IMG_7369.jpg', 'IMG_7373.jpg', 'IMG_7469.JPG',
    'IMG_7505.jpg', 'IMG_7508.jpg', 'IMG_7509.jpg', 'IMG_7525.jpg', 'IMG_7527.jpg', 'IMG_7559.JPG',
    'IMG_7578.jpg', 'IMG_7594.jpg', 'IMG_7729.jpg', 'IMG_7764.JPG', 'IMG_8175.JPG', 'IMG_8302.JPG',
    'IMG_8332.JPG', 'IMG_8436.JPG', 'IMG_8611.JPG', 'IMG_8661.JPG', 'IMG_8662.JPG', 'IMG_8675.jpg',
    'IMG_9697.JPG', 'IMG_9932.jpg', 'IMG_9933.jpg', 'IMG_9934.jpg', 'IMG_9935.JPG', 'IMG_9936.jpg',
    'IMG_9940.JPG', 'IMG_9942.JPG', 'IMG_9943.JPG', 'IMG_9944.JPG',
  );
}
...
function bgv_default_interior_exterior_sections() {
  return array(
    'exterior' => array(
      ...
      'images' => array(
        array(
          'src' => bgv_preferred_asset_uri_if_exists('assets/images/uploads/2022/04/gai1.jpg'),
          'alt' => '外観1',
        ),
        array(
          'src' => bgv_preferred_asset_uri_if_exists('assets/images/uploads/2022/04/gai2.jpg'),
          'alt' => '外観2',
        ),
      ),
    ),
    'interior' => array(
      ...
      'images' => array(
        array(
          'src' => bgv_preferred_asset_uri_if_exists('assets/images/uploads/2022/04/nai1.jpg'),
          'alt' => '内観1',
        ),
        array(
          'src' => bgv_preferred_asset_uri_if_exists('assets/images/uploads/2022/04/nai2.jpg'),
          'alt' => '内観2',
        ),
        array(
          'src' => bgv_preferred_asset_uri_if_exists('assets/images/uploads/2022/04/nai3.jpg'),
          'alt' => '内観3',
        ),
        array(
          'src' => bgv_preferred_asset_uri_if_exists('assets/images/uploads/2022/04/nai4.jpg'),
          'alt' => '内観4',
        ),
      ),
    ),
  ...
  <div class="photos interior-exterior-photos">
    <?php foreach ($sections as $section) : ?>
      ...
      <section class="photos-section">
        <h2 class="photos-section-title"><?php echo esc_html($section['title']); ?><span><?php echo esc_html($section['subtitle']); ?></span></h2>
...
function bgv_get_gallery_images($post_id = null) {
  ...
  if ($target_post_id && function_exists('get_field')) {
    for ($i = 1; $i <= count($default_gallery_files); $i++) {
      ...
      $url = bgv_preferred_asset_uri_if_exists($fallback_path);
      $alt = bgv_get_field('gallery_image_' . $i . '_alt', $default_alt, $target_post_id);
      if ($url === '') {
        continue;
      }
  ...
  foreach ($default_gallery_files as $index => $file) {
    $asset_path = 'assets/images/auto_gal/' . $file;
    if (! bgv_asset_exists($asset_path)) {
      continue;
    }
    $url = bgv_preferred_asset_uri($asset_path);
    $alt = 'ギャラリー' . ($index + 1);
    $images[] = array(
      'full_url' => $url,
      'thumb_html' => '<img src="' . esc_url($url) . '" alt="' . esc_attr($alt) . '" loading="lazy" decoding="async" />',
...
function bgv_render_gallery_grid_list($images, $lazy_from = 12) {
  if (! is_array($images) || empty($images)) {
    return false;
  }
  ...
  foreach ($images as $index => $image) {
    echo '<li' . ($index >= $lazy_from ? ' class="lazy"' : '') . '>';
    echo '<a href="' . esc_url($image['full_url']) . '" class="gallery" aria-label="' . esc_attr($image['alt']) . 'を拡大表示">';
    echo $image['thumb_html'];
    echo '</a>';
    echo '</li>';
  }
...
function bgv_get_home_ambience_images($post_id = null) {
  $sections = bgv_get_interior_exterior_sections($post_id);
  $images = array();
...
function bgv_sign_page_title_map() {
  return array(
    'kokuban' => array(
      'label' => '黒板メニュー',
      'breadcrumb' => '黒板メニュー',
      'image' => 'assets/images/assets/kokuban-title-sign-stylish.png',
      'width' => 2172,
      'height' => 724,
    ),
    'lunch' => array(
      'label' => 'ランチ',
      'breadcrumb' => 'Lunch',
      'image' => 'assets/images/assets/title-sign-lunch-stylish.png',
      'width' => 2180,
      'height' => 721,
    ),
    'dinner' => array(
      ...
      'image' => 'assets/images/assets/title-sign-dinner-stylish.png',
      'width' => 2180,
      'height' => 721,
    ),
    'kids' => array(
      'label' => 'お子様セット',
      'breadcrumb' => 'Kids Menu',
      'image' => 'assets/images/assets/title-sign-kids-stylish.png',
      'width' => 2180,
      'height' => 721,
    ),
    'drink' => array(
      'label' => 'ドリンク',
      'breadcrumb' => 'Drink Menu',
      'image' => 'assets/images/assets/title-sign-drink-stylish.png',
      'width' => 2180,
      'height' => 721,
    ),
    'wine' => array(
      'label' => 'ワインリスト',
      'breadcrumb' => 'Wine List',
      'image' => 'assets/images/assets/title-sign-wine-stylish.png',
      'width' => 2180,
      'height' => 721,
    ),
  );
...
function bgv_render_sign_page_title($key, $breadcrumb_label = '') {
  $map = bgv_sign_page_title_map();
  if (! isset($map[$key])) {
    bgv_render_page_title($breadcrumb_label ?: $key);
    return;
  }

  $definition = $map[$key];
  $label = $breadcrumb_label ?: $definition['breadcrumb'];
  $image_uri = bgv_preferred_asset_uri_if_exists($definition['image']);

  if (! $image_uri) {
    bgv_render_page_title($label ?: $definition['label']);
    return;
  }
  ?>
  <section id="pagetitle" class="sign-title-page">
    <h1 class="kokuban-sign-title">
      <img
        ...
        alt="<?php echo esc_attr($definition['label']); ?>"
        width="<?php echo esc_attr((string) $definition['width']); ?>"
        height="<?php echo esc_attr((string) $definition['height']); ?>"
        decoding="async"
        fetchpriority="high"
      />
    </h1>
  ...
  <section id="breadcrumb">
    <div class="container">
      <ul>
        <li><a href="<?php echo esc_url(home_url('/')); ?>" class="home"><span>HOME</span></a></li>
        <li><span class="fa fa-caret-right"></span><span><?php echo esc_html($label); ?></span></li>
      </ul>
    </div>
  </section>
  <?php
...
function bgv_static_default_content($filename) {
  ...
  $content = preg_replace_callback('/((?:href|src)=["\'])images\/([^"\']+)(["\'])/u', function($matches) {
    $relative_path = 'assets/images/' . ltrim($matches[2], '/');
    return $matches[1] . esc_url(bgv_preferred_asset_uri($relative_path)) . $matches[3];
  }, $content);
...
function bgv_default_kokuban_images() {
  return array(
    array('url' => bgv_preferred_asset_uri_if_exists('assets/images/assets/kokuban-menu-1.png'), 'width' => 945, 'height' => 705),
    array('url' => bgv_preferred_asset_uri_if_exists('assets/images/assets/kokuban-menu-2.png'), 'width' => 934, 'height' => 698),
  );
}
